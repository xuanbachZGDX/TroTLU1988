      ...
      await db.sequelize.transaction(async (transaction) => {
        ...
        if (post.status === "pending") {
          ...
          if (paymentTx) {
            ...
            if (user) {
              await db.User.update({ balance: (user.balance || 0) + paymentTx.amount }, { where: { id: post.userId }, transaction });
...
export const getPostHistoryService = (postId, actor) =>
  new Promise(async (resolve, reject) => {
    try {
      const where = actor?.role === "admin" ? { id: postId } : { id: postId, userId: actor?.id };
      const post = await db.Post.findOne({ where });
      if (!post) return resolve({ err: 1, msg: "Không tìm thấy bài đăng hoặc bạn không có quyền xem lịch sử" });
