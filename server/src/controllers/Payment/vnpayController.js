...
export const createPaymentUrl = async (req, res) => {
  ...
  try {
    await db.Transaction.create({
      ...
      amount: amount,
      type: "deposit",
      content: `Nạp tiền vào tài khoản TroTLU1988.com`,
      status: "pending",
    });
  } catch (err) {
      ...
      await db.sequelize.transaction(async (t) => {
        ...
        if (transaction && transaction.status === "pending") {
          if (rspCode === "00" && Number(transaction.amount) === amount) {
            ...
            res
              .status(200)
              .json({ code: rspCode, msg: "Thanh toán thành công" });
          } else {
            await db.Transaction.update(
              { status: "failed" },
              { where: { id: orderId }, transaction: t },
            ...
            res.status(200).json({
              code: rspCode,
              msg: "Giao dịch thất bại hoặc sai lệch số tiền",
            });
          }
        } else {
          res.status(200).json({
            code: rspCode,
            msg: "Giao dịch đã được xử lý hoặc không tồn tại",
          });
        }
      });
    } else {
