# Kịch Bản Kiểm Thử Chi Tiết Hệ Thống - PhongTro123

Tài liệu này đặc tả chi tiết kịch bản kiểm thử (Test Cases) cho toàn bộ các chức năng thuộc hệ thống PhongTro123, bao gồm cả Client, Server và Module Crawler.
...
## 3. Feature: Deposit & Transactions (Nạp tiền & Lịch sử giao dịch qua VNPay)
...
## 4. Feature: Post Management - Landlord (Quản lý tin đăng - Chủ trọ)
...
## 5. Feature: Extend & Upgrade Post (Gia hạn & Nâng cấp tin đăng)
...
## 6. Feature: Search & Filter (Tìm kiếm & Bộ lọc)
...
## 7. Feature: Wishlist (Tin đăng đã lưu / yêu thích)
...
## 8. Feature: Contact & Inquiries (Gửi liên hệ / góp ý)
...
## 9. Feature: Admin Dashboard & Management (Trang quản trị & Cấu hình Admin)
...
## 10. Feature: Notification System (Hệ thống thông báo)
...
### [TC_POST_003B] Chỉnh sửa tin bị Admin từ chối và gửi duyệt lại (Trừ phí đăng lại)
...
* **Steps:**
  ...
  2. Nhấn nút "Sửa" và cập nhật thông tin bài đăng cho hợp lệ.
  3. Nhấn "Lưu cập nhật và gửi duyệt".
* **Test Data:** 
  * Tiêu đề mới: "Cho thuê phòng trọ giá tốt chính chủ..."
  * Gói tin ban đầu: VIP 3 (10,000đ/ngày)
  * Thời hạn: 10 ngày (Phí đăng lại: 100,000đ)
  * Số dư ví hiện tại: 120,000đ
* **Expected:** 
  1. Hệ thống tính toán lại phí và trừ `100,000đ` từ số dư ví của chủ trọ.
  2. Tạo giao dịch trừ phí bài đăng (`payment`) mới.
  3. Trạng thái tin đăng chuyển về `pending` chờ Admin duyệt lại.
* **Actual:** Đạt
...
### [TC_WSH_004] Đồng bộ danh sách "Tin đã lưu" sau khi Đăng nhập tài khoản
...
* **Test Data:** Không có.
* **Expected:** Danh sách tin đã lưu trước khi đăng nhập không bị mất đi, hệ thống đồng bộ dữ liệu lưu tạm từ LocalStorage vào danh sách tin yêu thích của tài khoản vừa đăng nhập.
* **Actual:** Đạt

---

### [TC_AUTH_012] Quên mật khẩu - Gửi yêu cầu khôi phục mật khẩu thành công
* **Precondition:** Email nhập vào đã được đăng ký tài khoản trên hệ thống.
* **Steps:**
  1. Nhấp vào đường dẫn "Quên mật khẩu" tại trang Đăng nhập.
  2. Nhập địa chỉ email đăng ký tài khoản.
  3. Bấm "Gửi yêu cầu".
* **Test Data:** Email: `landlord_test@gmail.com`
* **Expected:** 
  1. Hệ thống xử lý thành công, gửi đường dẫn reset mật khẩu qua email cho người dùng.
  2. Hiển thị thông báo "Yêu cầu khôi phục mật khẩu đã được gửi, vui lòng kiểm tra hộp thư email của bạn".
* **Actual:** Đạt
...
### [TC_AUTH_014] Chặn người dùng thông thường truy cập API Admin (API-level Route Guard)
* **Precondition:** Đăng nhập bằng tài khoản có vai trò `role = "user"`.
* **Steps:**
  1. Sử dụng công cụ (Postman/cURL hoặc gọi fetch từ console) gửi request GET đến API admin `/api/v1/admin/users`.
  2. Đính kèm token JWT của user thông thường vào header Authorization.
* **Test Data:** Authorization Token của tài khoản `user`.
* **Expected:** API trả về mã trạng thái HTTP `403 Forbidden` kèm thông báo lỗi "Bạn không có quyền truy cập chức năng này", không trả về danh sách người dùng.
* **Actual:** Đạt
...
### [TC_POST_010] Đăng tin bởi tài khoản Admin tự động hiển thị không qua kiểm duyệt
* **Precondition:** Đăng nhập bằng tài khoản có vai trò `role = "admin"`, số dư ví đủ.
* **Steps:**
  1. Truy cập trang "Đăng tin mới".
  2. Nhập đầy đủ thông tin hợp lệ và hoàn tất thanh toán.
  3. Kiểm tra trạng thái tin đăng vừa tạo trong Database.
* **Test Data:** Tài khoản Admin, gói tin bất kỳ.
* **Expected:** Tin đăng được tạo trực tiếp với trạng thái `status = "active"`, hiển thị ngay lập tức ngoài trang chủ tìm kiếm mà không qua trạng thái `pending`.
* **Actual:** Đạt
...
## 11. Feature: UI/UX & Responsive Design (Giao diện, Trải nghiệm & Đáp ứng)
...
### [TC_ADM_009] Thay đổi mật khẩu tài khoản Admin từ trang quản trị (Admin Password Update)
* **Precondition:** Đang đăng nhập tài khoản Admin và ở trang quản trị Admin.
* **Steps:**
  1. Vào mục "Cài đặt tài khoản" của Admin.
  2. Nhập mật khẩu cũ, mật khẩu mới và xác nhận mật khẩu mới.
  3. Bấm "Cập nhật".
* **Test Data:** Mật khẩu cũ: `admin123456`, Mật khẩu mới: `adminSecure99!`.
* **Expected:** Đổi mật khẩu thành công, phiên làm việc hiện tại của Admin bị vô hiệu hóa, hệ thống chuyển hướng về trang đăng nhập của Admin và yêu cầu xác thực lại bằng mật khẩu mới.
* **Actual:** Đạt
...
### [TC_SRCH_010] Nhập khoảng giá / diện tích có giới hạn dưới lớn hơn giới hạn trên (Invalid Range Boundary Filter)
* **Precondition:** Đang ở màn hình bộ lọc nâng cao (lọc tự chọn).
* **Steps:**
  1. Chọn bộ lọc giá trị tự chọn với giá trị từ `5 triệu` đến `3 triệu` (hoặc diện tích từ `50 m2` đến `30 m2`).
  2. Bấm nút "Áp dụng lọc".
* **Test Data:** Giá trị min = 5, giá trị max = 3.
* **Expected:** Hệ thống hiển thị cảnh báo validation ngay lập tức trên giao diện: "Giá trị tối thiểu không được lớn hơn giá trị tối đa" và chặn không cho thực hiện truy vấn lỗi.
* **Actual:** Đạt
