...
const AdminBellNotification = () => {
  ...
  const fetchNotifs = async () => {
    try {
      const res = isAdmin
        ? await apiGetAdminNotifications()
  ...
  const handleItemClick = async (n) => {
    ...
    if (!n.postId) {
      const titleLower = n.title?.normalize("NFC").toLowerCase() || "";
      const isKyc =
      ...
      if (isAdmin) {
        ...
        if (isKyc) {
          navigate(`/${path.SYSTEM}/${path.USER_KYC}`);
        } else {
    ...
    if (isAdmin) {
      setHistoryPost({
        id: n.postId,
        overview: { code: n.postId?.slice(0, 8).toUpperCase() },
      });
      ...
      if (response?.data?.err === 0) {
        ...
        Swal.fire(
          "Không tìm thấy",
          "Chưa có dữ liệu lịch sử cho tin đăng này.",
          "info",
        );
      }
    } else {
  ...
  return (
    <div className="relative flex items-center" ref={bellRef}>
      ...
      >
        ...
        />
        <span className="text-[11px] font-bold mt-0.5 whitespace-nowrap uppercase tracking-tighter">
          Thông báo
        </span>
        {unreadCount > 0 && (
        ...
        <div className="absolute top-[calc(100%+15px)] right-0 w-[320px] bg-white rounded-2xl shadow-2xl border border-gray-100 py-3 z-50 flex flex-col gap-2">
              ...
              notifications.slice(0, 5).map((n) => (
                ...
                >
                  ...
                  <div className="flex-1 flex flex-col gap-0.5 min-w-0">
                    <span
                      className={`text-[11px] truncate ${n.isRead ? "text-gray-700" : "font-bold text-gray-900"}`}
                    >
                      {n.title}
                    </span>
                  ...
                  {!n.isRead && (
                    <span className="w-1.5 h-1.5 bg-blue-500 rounded-full flex-none mt-1.5"></span>
                  )}
                </div>
              ))
            )}
          ...
          {isAdmin && (
            <button
              onClick={() => {
                setIsOpen(false);
                navigate(`/${path.ADMIN}/${path.ADMIN_NOTIFICATIONS}`);
              }}
