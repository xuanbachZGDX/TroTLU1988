...
const UserPostRow = ({
  ...
  const isActive =
    ...
    !isArchived &&
    !isBlocked &&
    checkStatus(item?.overview?.expired) === "Đang hoạt động";
  ...
  const statusColor = isBlocked
    ? "text-red-700 bg-red-100 border border-red-300 font-bold"
    : isRejected
  ...
  const renderPackage = (bonus) => {
    const baseClass =
      "whitespace-nowrap inline-block text-center px-2.5 py-1 rounded text-[10px] font-bold uppercase shadow-sm";
    if (!bonus) {
      return (
    ...
    if (bonusStr.includes("5")) {
      return (
        <span className={`${baseClass} bg-red-600 text-white animate-pulse`}>
          VIP Nổi bật
        </span>
      );
    }
    if (bonusStr.includes("4")) {
      return (
        <span className={`${baseClass} bg-pink-500 text-white`}>Tin VIP 1</span>
      );
    }
    if (bonusStr.includes("3")) {
      return (
        <span className={`${baseClass} bg-orange-500 text-white`}>
          Tin VIP 2
        </span>
      );
    }
    if (bonusStr.includes("2")) {
      return (
        <span className={`${baseClass} bg-blue-500 text-white`}>Tin VIP 3</span>
      );
    }
    return (
      <span className={`${baseClass} bg-gray-100 text-gray-500`}>
  ...
  return (
    <>
        ...
        <tr className="border-t border-red-100 bg-red-50">
          <td colSpan="9" className="px-4 py-2">
            <div className="flex items-center gap-2 text-red-700 text-xs">
              ...
              <span>
                <strong>Tin đăng đã bị khóa bởi Admin.</strong> Lý do:{" "}
                <strong className="underline italic text-red-900">
                  {item.note || "Vi phạm quy chế đăng tin"}
                </strong>
        ...
        <tr className="border-t border-red-100 bg-red-50">
          <td colSpan="9" className="px-4 py-2">
            <div className="flex items-center gap-2 text-red-700 text-xs">
              ...
              <span>
                <strong>Tin bị từ chối.</strong> Lý do:{" "}
                <strong className="underline italic text-red-900">
                  {item.note || "Nội dung hoặc hình ảnh chưa phù hợp"}
                </strong>
                . Hãy nhấn <strong>Sửa & Gửi lại</strong> bên dưới để chỉnh sửa
                và gửi duyệt lại.
              </span>
            </div>
          </td>
      ...
      <tr
        className={`border-t transition-colors text-xs ${isRejected || isBlocked ? "bg-red-50/50 hover:bg-red-50" : "border-gray-100 hover:bg-gray-50"}`}
      >
        <td className="px-4 py-4 font-mono text-gray-600">
          {item?.overview?.code || item?.id?.slice(0, 8)}
        </td>
        ...
        <td
          className="px-4 py-4 max-w-[180px] font-medium line-clamp-1"
          title={item.title}
        >
          {item.title}
        </td>
        <td className="px-4 py-4">{renderPackage(item?.overview?.bonus)}</td>
        ...
        <td className="px-4 py-4 text-center">
          <span
            className={`text-[11px] font-bold px-3 py-1 rounded-full whitespace-nowrap inline-block ${statusColor}`}
          >
            {statusLabel}
          </span>
        </td>
