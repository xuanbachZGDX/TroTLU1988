...
const EditAccount = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (payload.email && !emailRegex.test(payload.email)) {
      setInvalidFields([
        { name: "email", message: "Email không đúng định dạng" },
      ]);
    ...
    const isChanged =
      ...
      payload.phone !== (currentData?.phone || "");
  ...
  const handleChangePassword = async () => {
    const { value: formValues } = await Swal.fire({
      ...
      `,
      didOpen: () => {
        const cb = document.getElementById("show-password"),
          p0 = document.getElementById("swal-input-old"),
          p1 = document.getElementById("swal-input-new"),
          p2 = document.getElementById("swal-input-confirm");
      ...
      preConfirm: () => {
        const p0 = document.getElementById("swal-input-old").value,
          p1 = document.getElementById("swal-input-new").value,
          p2 = document.getElementById("swal-input-confirm").value;
        if (!p0 || !p1 || !p2) {
          Swal.showValidationMessage("Vui lòng nhập đầy đủ!");
          return false;
        }
        if (p1.length < 6) {
          Swal.showValidationMessage("Mật khẩu mới tối thiểu 6 ký tự!");
          return false;
        }
        if (p1 === p0) {
          Swal.showValidationMessage("Mật khẩu mới trùng mật khẩu cũ!");
          return false;
        }
        if (p1 !== p2) {
      ...
      <div className="w-3/5 py-6 flex flex-col gap-5">
        ...
        <div className="flex flex-row items-center">
          ...
          <div className="flex flex-auto items-center justify-between bg-gray-50 p-2 rounded-md border border-gray-200">
            <span className="text-gray-400 font-mono tracking-widest ml-2">
              ••••••••
            </span>
            <button
              type="button"
              className="text-blue-600 font-medium text-sm px-3 py-1 bg-white border border-blue-200 rounded"
