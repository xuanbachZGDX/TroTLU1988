...
const InvoiceModal = ({ isOpen, onClose, tx, userName }) => {
  ...
  const typeColor = tx.type === 'deposit' || tx.type === 'refund' ? 'text-emerald-600 bg-emerald-50 border-emerald-200' : 'text-rose-600 bg-rose-50 border-rose-200';
  const amountColor = tx.type === 'deposit' || tx.type === 'refund' ? 'text-emerald-600' : 'text-rose-600';
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto no-print-backdrop">
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden border border-gray-100 transform transition-all animate-fade-in no-print-modal">
        ...
        <div id="print-invoice-area" className="p-8 bg-white text-gray-800">
          <div className="flex justify-between items-start border-b border-blue-500 pb-5 mb-6">
            <div>
              <div className="text-2xl font-extrabold text-blue-700 tracking-tight">TLU<span className="text-blue-400">.com</span></div>
            </div>
            <div className="text-right text-[11px] text-gray-500 leading-relaxed max-w-[240px]">
              <strong className="text-gray-700">KÊNH THÔNG TIN PHÒNG TRỌ HÀNG ĐẦU</strong><br/>
              Website: www.tlu.com<br/>
              Hotline: 1900 1234<br/>
              Email: support@tlu.com
            </div>
          </div>

          <h2 className="text-xl font-bold text-center text-gray-900 mb-6 uppercase tracking-wider">Biên Lai Xác Nhận Giao Dịch</h2>

          <div className="grid grid-cols-2 gap-4 bg-slate-50 border border-slate-100 p-4 rounded-xl mb-6 text-sm">
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Mã giao dịch</span>
              <span className="font-mono font-semibold text-slate-700 break-all">{tx.id}</span>
            </div>
            <div className="flex flex-col gap-1">
            ...
            <div className="flex flex-col gap-1 col-span-2">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Khách hàng thực hiện</span>
          ...
          <div className="border-t border-b border-dashed border-gray-200 py-4 mb-6 text-sm flex flex-col gap-3">
            <div className="flex justify-between">
              <span className="text-gray-500 font-medium">Loại giao dịch:</span>
            ...
            <div className="flex justify-between items-start">
              ...
              <span className="font-semibold text-gray-800 text-right max-w-[280px]">{tx.content}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500 font-medium">Trạng thái hệ thống:</span>
              <span className="font-bold text-emerald-600">Thành công</span>
            </div>
          </div>

          <div className={`flex justify-between items-center p-4 rounded-xl mb-8 border ${typeColor}`}>
            <span className="text-xs font-bold uppercase tracking-wider">Số tiền giao dịch</span>
            <span className={`text-2xl font-black ${amountColor}`}>{amountSign}{formattedAmount} đ</span>
          </div>

          <div className="flex justify-between mt-8 relative">
            <div className="text-center w-[180px]">
              <span className="font-bold text-xs uppercase text-gray-600 block mb-14">Khách hàng</span>
              <span className="font-bold text-xs text-gray-800 block">{customerName}</span>
              <span className="text-[10px] text-gray-400 block">(Ký và ghi rõ họ tên)</span>
            </div>
            
            <div className="text-center w-[180px] relative">
              <span className="font-bold text-xs uppercase text-gray-600 block mb-14">Đại diện TLU.com</span>
              <span className="font-bold text-xs text-gray-800 block">Ban Quản Trị TLU</span>
              <span className="text-[10px] text-gray-400 block">Hệ thống hóa đơn điện tử</span>
          ...
          <div className="mt-12 text-center text-[10px] text-gray-400 leading-relaxed border-t border-gray-100 pt-4">
            Cảm ơn quý khách đã sử dụng dịch vụ tại TLU.com!<br/>
        ...
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-3 no-print">
          ...
          <button 
            onClick={handlePrint}
            className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold text-sm rounded-lg transition-colors shadow-sm flex items-center gap-1.5"
          >
