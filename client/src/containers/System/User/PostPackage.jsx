...
import { apiGetPostById, apiExtendPost } from "../../../services/postService";
import Swal from "sweetalert2";
import * as actions from "../../../store/actions";

const PostPackage = ({ payload: propsPayload, setPayload: propsSetPayload }) => {
  ...
  const handleConfirmExtension = async () => {
    if (!isEnoughBalance) {
      return Swal.fire("Thông báo", "Số dư không đủ, vui lòng nạp thêm tiền để hoàn tất thanh toán", "warning");
    }
  ...
  return (
    <div className={`bg-white rounded-md shadow-sm border border-gray-200 p-6 md:p-10 ${isExtension ? 'm-6' : ''} animate-slide-right`}>
      {isExtension && (
        <div className="flex items-center gap-2 mb-6 text-gray-500 hover:text-blue-600 cursor-pointer w-fit" onClick={() => navigate(-1)}>
      ...
      <h2 className="text-2xl font-semibold mb-8 text-gray-800">{isExtension ? "Gia hạn tin đăng" : "Thanh toán & Đăng tin"}</h2>
          ...
          <div className="bg-gray-50 rounded-md border border-gray-200 p-6 sticky top-20">
            ...
            <div className="flex flex-col gap-3 text-sm mb-4">
              <div className="flex justify-between">
                ...
                <span className="font-medium text-right">{packages.find(p => p.code === (payload?.star || "0"))?.value}</span>
              </div>
              <div className="flex justify-between">
            ...
            <div className="bg-white p-3 rounded border border-gray-200 text-sm">
              ...
              {!isEnoughBalance && (
                <div className="text-red-500 mt-2 pt-2 border-t text-xs">
                  Số dư không đủ, vui lòng <Link to={`/${path.SYSTEM}/${path.DEPOSIT}`} className="text-blue-600 underline font-medium">Nạp thêm tiền</Link> để hoàn tất thanh toán
                </div>
              )}
            </div>
